...
### Phase 2 — actions / AI
- `Action.swift` — `Action { name, icon, steps: [Step] }`, `Step { type: .script|.ai, enabled, script, prompt, model? }` (generalizes AI Replace's `Preset`). `Action.defaults()` seeds the menu (incl. image-friendly AI actions "Extract text" / "Describe image"). `kindTag` → SCRIPT/AI.
