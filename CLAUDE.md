...
- `ClipboardWatcher.swift` — `Timer` polling `changeCount` (~0.3s). Privacy filter → classify image|text → capture frontmost app → `store.add`. `markSelfCopy(hash:)` self-write guard so a paste-of-existing bumps rather than re-captures (AI/script results are *not* marked → re-enter as fresh top items). Image takes priority over text; TIFF/PNG normalized to PNG.
...
- `HotkeyManager.swift` — Carbon multi-hotkey (copied from AI Replace). ⌘⇧V `showHistory` id=1; ⌃⇧V `transformSelection` id=2 gated behind `registerSelection` (off until that commit).
