...
- `AIProvider.swift` — **multi-provider routing.** `AIProvider.provider(for:)` picks by model-id prefix (`ollama:*` → `.ollama`, `claude*` → `.anthropic`, else `.openai`) — no schema migration, OpenAI stays default. `AIClient` facade dispatches to `OpenAIClient`/`AnthropicClient`/`OllamaClient`; cloud providers read a keychain key (`keychain` is `nil` for local `.ollama` and the facade skips the key check), local reads the configured base URL closure. Add a provider here (case + keychain/dispatch).  Unit-tested.
- `OpenAIClient.swift` / `AnthropicClient.swift` / `OllamaClient.swift` — raw `URLSession` clients (chat-completions / Messages API `https://api.anthropic.com/v1/messages` / local `\(baseURL)/api/generate`). Ollama is **local, no auth, no key**, `stream:false`, 120s timeout, `ollama:` prefix stripped before send; friendly `notRunning`/`modelNotFound` (404) errors. `ModelCatalog` (in `OpenAIClient.swift`) groups `.openAI`/`.anthropic`/`.ollama` (`ollama:llama3.2`/`qwen2.5`/`mistral`); `.all = openAI + anthropic + ollama`; `.default = gpt-4o-mini`.
