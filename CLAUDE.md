...
- `ActionRunner.swift` — `@MainActor`; threads text through enabled steps in order (`.script` → `ScriptRunner`, `.ai` → `AIClient` facade). Script/AI runners are **injectable** so tests are hermetic. A throwing step aborts before paste. `ActionError.missingApiKey(AIProvider)` names the right provider. Unit-tested.
- `AIProvider.swift` — **multi-provider routing.** `AIProvider.provider(for:)` picks by model-id prefix (`claude*` → `.anthropic`, else `.openai`) — no schema migration, OpenAI stays default. `AIClient` facade reads the matching provider's keychain key and dispatches to `OpenAIClient`/`AnthropicClient`. Add a 3rd provider here (case + keychain + dispatch). Unit-tested.
- `OpenAIClient.swift` / `AnthropicClient.swift` — raw `URLSession` clients (chat-completions / Messages API `https://api.anthropic.com/v1/messages`, `x-api-key` + `anthropic-version: 2023-06-01`, `max_tokens` required, no thinking/sampling). `ModelCatalog` (in `OpenAIClient.swift`) groups `.openAI`/`.anthropic`; `.all = openAI + anthropic`; `.default = gpt-4o-mini`.
...
- Action menu now shows built-ins → `Saved actions` divider → user actions (text clips only, with SCRIPT/AI tags). Running a saved action threads the clip text through `ActionRunner` and pastes the result (which re-enters history as a fresh top item — *not* marked self-copy). Settings General gains OpenAI **and Anthropic** sections (API key + Show/Test/Save each) + a Default-model picker grouped by provider. `AppSettings` extended with `defaultModel` + `actions`.
