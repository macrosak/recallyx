...
- `HistoryStore.swift` — `@MainActor ObservableObject` owning the on-disk history. `add` (dedupe-bump or insert) / `bump` / `delete` / `clear`. Cap eviction (enforced on load too — `cap`'s `didSet` doesn't fire in `init`), atomic save (temp + `replaceItemAt`), debounced writes, reseed-on-corrupt, orphan reconciliation (skipped on the corrupt-reseed path so the backed-up index's PNGs survive next to the `.corrupt-*` file).
