...
- **`make-social.sh [SCREENSHOT]`** — composites a screenshot (default: the history shot) into the transparent rounded hole of `docs/recallyx-banner-template-2560x1280.png` → `docs/recallyx-social.png` (full res, used by the README) + `docs/recallyx-social-1280x640.png` (half size, for GitHub's ~1 MB social-preview upload). Finds the hole by scanning for transparent pixel runs, so the template slot can move or resize freely.
